...
module I2P
  ...
  #
  # This is an implementation of the SAM V3 protocol, which is available
  # since I2P release 0.7.3.
  #
  ...
  module SAM
    ...
    class Client
      ...
      def lookup_name(name)
        ...
        Key::Public.parse(read_reply[:value])
      end
      ...
      #
      # @return [Array(Key::Private, Key::Public)]
      def generate_dest
        send_msg(:dest, :generate)
        keys = read_reply
        [Key::Private.parse(keys[:priv]), Key::Public.parse(keys[:pub])]
      end
